...
class DefaultMigrator extends events_1.EventEmitter {
    constructor(migrationProvider, dataAccessProvider, seed) {
        super();
        this.seeder = null;
        this.seeder = seed;
    ...
    seed() {
        return __awaiter(this, void 0, void 0, function* () {
            if (this.seeder) {
                const dataAccess = yield this.getDataAccess();
                const d = formatting_1.formatDate(new Date());
                const id = `${d}--seed`;
                this.emit('seeding');
                // Run the seed in a migration "context" so we get auto-rollbacks if the
                // seed fails...
                try {
                    yield dataAccess.migrate(id, q => this.seeder.run(q));
                }
                catch (error) {
                    yield dataAccess.close();
                ...
                // migration table.
                yield dataAccess.rollback(id, q => Promise.resolve());
                this.emit('seed');
                yield dataAccess.close();
            }
        });
