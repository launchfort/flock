...
class Migrator extends EventDispatcher {
  ...
  /**
   * Retrives all the migrated migration IDs.
   *
   * @return {Promise<string[]>}
   */
  async getMigratedMigrationIds () {
    const { context, disconnect } = await this.connect()
    const ids = await Promise.all(this.migrationInteractors.map(x => {
      return x.hasRun(context).then(hasRun => {
        return hasRun ? x.id : null
      })
    }))
    await disconnect()
   ...
   * @return {Promise<string[]>}
   */
  async getUnmigratedMigrationIds () {
    const migratedIds = await this.getMigratedMigrationIds()
    return this.migrationIds.filter(x => migratedIds.indexOf(x) < 0)
  }
