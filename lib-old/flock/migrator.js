 ...
 * @prop {ConnectionContext} context
 */
...
class Migrator extends EventDispatcher {
  ...
  /**
   * Determines if the migration table has been created and ready.
   * @param {ConnectionContext} context
   */
  async isMigrationTableReady (context) {
    throw new Error('Unimplemented')
  }
  ...
  async inspectMigrations () {
    ...
    try {
      const ready = await this.isMigrationTableReady(context)
      if (!ready) {
        return this.migrationInteractors.map(x => {
          return { id: x.id, migrated: false }
        })
      }

      const result = await Promise.all(this.migrationInteractors.map(async (x) => {
