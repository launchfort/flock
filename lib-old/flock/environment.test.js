...
describe('Environment', function () {
  describe('#enumerateDrivers', function () {
    it('should enumerate the built in drivers', async function () {
      ...
      assert.deepStrictEqual(drivers, [
        {
          id: 'flock:lib/drivers/dummy',
          description: 'Flock database driver for testing',
          name: 'flock-dummy'
        },
        {
  ...
  describe('#generateMigrationBasename', function () {
    ...
    it('should generate a new migration file name (starting a day\'s sequence)', async function () {
      ...
      assert.strictEqual(basename, `${timecode}--001--create-table--message.js`)
    })
    ...
    it('should generate a new migration file name (starting a day\'s sequence)', async function () {
      ...
      await writeFile(path.join(migrationDir, `${timecode}--001--create-table--message.js`), '', { encoding: 'utf8' })
      ...
      try {
        ...
        assert.strictEqual(basename, `${timecode}--002--create-table--message.js`)
      } finally {
        fs.unlinkSync(path.join(migrationDir, `${timecode}--001--create-table--message.js`))
      }
    })
    ...
    describe('#getTemplateFileName', function () {
      ...
      it('should retrieve the template file name from an installed database driver module', function () {
        const env = new Environment()
      ...
      it('should retrieve the template file name from a built-in database driver module', function () {
        const env = new Environment()
        const id = 'flock:lib/drivers/pg'
        const templateFileName = env.getTemplateFileName('create-table', id)
        assert.strictEqual(templateFileName,
          path.resolve(__dirname, '../drivers/pg/templates/create-table.ejs')
        )
      })
    })
